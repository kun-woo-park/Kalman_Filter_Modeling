...
if __name__ == "__main__":
    ...
    if (os.path.exists("mean_noised_" + num_of_features + ".npy") and os.path.exists(
        ...
        data_train = np.loadtxt("kf_train_noised_" + num_of_features + ".csv", delimiter=",", dtype=np.float32)
        data_val = np.loadtxt("kf_val_noised_" + num_of_features + ".csv", delimiter=",", dtype=np.float32)
        ...
        np.save("mean_noised_" + num_of_features + ".npy", mean)
        np.save("std_noised_" + num_of_features + ".npy", std)
    else:
        mean = np.load("mean_noised_" + num_of_features + ".npy").tolist()
        std = np.load("std_noised_" + num_of_features + ".npy").tolist()
        data_train = np.loadtxt("kf_train_noised_" + num_of_features + ".csv", delimiter=",", dtype=np.float32)
        data_val = np.loadtxt("kf_val_noised_" + num_of_features + ".csv", delimiter=",", dtype=np.float32)
