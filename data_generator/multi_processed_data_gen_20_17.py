...
import pandas as pd
import os
from data_gen_20_17 import uni_data_generator
...
if __name__ == "__main__":
    ...
    if noise:
        ...
        file_name = ""
    if os.path.exists("kf_train"+file_name+"340.csv") is False:
        # train data
        ...
        res = np.array(res)
        df = pd.DataFrame(res)
        df.to_csv("kf_train"+file_name+"_340.csv", header=False, index=False)
        print("Train data generation complete")
    if os.path.exists("kf_val"+file_name+"_340.csv") is False:
        # validation data
        ...
        res = np.array(res)
        df = pd.DataFrame(res)
        df.to_csv("kf_val"+file_name+"_340.csv", header=False, index=False)
